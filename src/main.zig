...
pub fn main() !void {
    ...
    const allocatedMem = win.mem.VirtualAllocEx(hProcess, null, DLL_PATH.len + 1, (win.mem.VIRTUAL_ALLOCATION_TYPE{ .COMMIT = 1, .RESERVE = 1 }), win.mem.PAGE_READWRITE);
    ...
    _ = win.system.diagnostics.debug.WriteProcessMemory(hProcess, allocatedMem, DLL_PATH.ptr, DLL_PATH.len + 1, null);
